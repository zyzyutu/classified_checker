# 涉密信息综合检查系统

## 项目简介
基于 Python + tkinter 开发的本地涉密信息综合检查工具，支持网页、数据库、文件、图片四大模块的涉密关键词自动检查，并生成 Markdown 报告。

**核心创新**：集成 Ollama 本地大模型进行语义级涉密信息检测，相比传统正则匹配，能理解语义、识别变体表达、过滤误报。

## 功能特性

### 大模型语义检查（核心创新）
- 通过 Ollama 集成本地大模型（如 Qwen2.5、Llama3 等）
- 语义理解：不仅匹配关键词字面，还能理解"涉密文件"、"机密级"等语义相关表达
- 变体识别：能识别"机 密"（中间有空格）、"秘~密"（中间有符号）等变形写法
- 误报过滤：能区分"保密协议"（涉密）和"保密工作"（正常语境）
- 可切换回传统正则匹配模式（兼容方案）

### 网页检查
- BFS 全站遍历，可配置爬取深度（0~10）
- 多线程并行爬取（可配置 1~16 线程）
- 增量检测：利用 ETag / Last-Modified / 内容 MD5 哈希跳过未变化的页面
- 缓存涉密标记，增量模式下涉密页面仍会在报告中展示

### 数据库检查
- GUI 界面输入数据库连接信息（地址/用户名/密码）
- **多数据库类型支持**：MySQL、SQL Server、PostgreSQL、SQLite（适配器模式）
- **多连接同时检查**：可添加多条数据库连接，一次检查全部
- 支持检查全部数据库或指定单个数据库
- 两轮筛选：SQL LIKE 粗筛 + 大模型/正则精匹配
- 自动检测表主键作为记录标识
- 跳过数值字段，同行关键词去重合并

### 文件检查
- 支持 TXT、DOC、DOCX、XLS、XLSX、PPT、PPTX、PDF
- 支持 ZIP、RAR、7Z 压缩包递归解压检查（最大嵌套 3 层）
- Magic Number 文件头校验真实类型
- **加密文件识别与解密**：支持 Office/ZIP/RAR/7Z/PDF 加密检测
- **密码输入弹窗**：检测到加密文件后弹窗让用户输入密码，支持重试
- 使用 msoffcrypto-tool 解密 Office/PDF 加密文件
- 隐藏文件检测、损坏文件诊断
- COM 应用实例缓存，RLock 可重入锁保证线程安全
- 多线程并行检查

### 图片检查
- 本地 OCR（RapidOCR），支持 GPU 加速（DirectML/CUDA）
- 多线程并行 OCR
- 文字位置检测（上部/中部/下部）
- OCR 结果缓存，置信度过滤（阈值 0.7）

### 报告输出
- 自动生成 Markdown 格式报告
- 包含学号和 GitHub 链接
- 标注检测方法（大模型/正则）
- 长文本自动截取关键词周围上下文
- 加密文件解密状态展示

### 界面
- 深色主题 GUI，字体清晰可读
- 大模型配置区：模型选择、连接状态显示
- 多数据库连接列表管理（添加/删除）
- 数据库类型切换时自动适配（SQLite 隐藏用户名密码）
- 支持立即停止、断点续查
- 加密文件密码输入弹窗（大尺寸，兼容深色主题）

## 快速开始

```bash
# 1. 安装依赖
pip install -r requirements.txt

# 2. 安装 Ollama（大模型检查必需）
# 下载: https://ollama.ai
# 拉取模型: ollama pull qwen2.5:7b
# 启动服务: ollama serve

# 3. 运行程序
python main.py
```

## 依赖库

| 库 | 用途 |
|---|------|
| requests | HTTP 请求（含 Ollama API 调用） |
| beautifulsoup4 | HTML 解析 |
| pymysql | MySQL 连接（可选） |
| pymssql | SQL Server 连接（可选） |
| psycopg2-binary | PostgreSQL 连接（可选） |
| python-docx | DOCX 读取 |
| openpyxl | XLSX 读取 |
| python-pptx | PPTX 读取 |
| PyPDF2 / pdfplumber | PDF 读取 |
| xlrd | XLS 读取 |
| pywin32 | WPS/Word COM 接口 |
| rapidocr-onnxruntime | OCR 识别 |
| rarfile | RAR 解压 |
| py7zr | 7Z 解压 |
| msoffcrypto-tool | 加密 Office/PDF 解密 |
| Ollama | 本地大模型推理（需单独安装） |

## 开发环境
- Python 3.11
- Windows 11
- Ollama + Qwen2.5:7b（大模型检查）
